...
- buy 只能针对 candidates 名单中的股票；对**已持仓且浮盈**的标的出 buy 表示金字塔补仓
- sell 只能针对当前持仓
- 必须参考历史教训，避免重复犯错
- 实际买入数量和止损位由系统按风险预算（0.75%净值/笔、2×ATR止损）自动计算，position_pct 仅作意向上限
- 置信度标定：5=中性不确定；6=值得小仓位一试；7-8=较有把握；9-10=极强信号（极少使用）。不要为了显得谨慎而都打 5 分"""
